    ...
    async analyzeEvent() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                let payload = null;
                try { payload = JSON.parse(errorText); } catch {}
                const msg = payload?.error || `Server returned ${response.status}: ${response.statusText}`;
                throw new Error(msg);
            }
            ...
            while (true) {
                ...
                if (statusResp.ok && statusJson && statusJson.success && statusJson.analysis) {
                    ...
                    this.lastKnowledgeBase = statusJson.kb || null;
                    const roleMeta = statusJson.analysis?._meta || {};
                    this.displayResults(statusJson.analysis, statusJson.generated_by, null, this.lastKnowledgeBase);
                    window.dispatchEvent(new CustomEvent('analysis:completed', {
                        detail: {
                            analysis: statusJson.analysis,
                            input: eventDescription,
                            role_context_summary: roleMeta.role_context_summary || null,
                        }
                    }));
                    this.loadPastSearches();
                    break;
                ...
                if (!statusResp.ok) {
                    const msg = statusJson?.error || `Analysis failed (HTTP ${statusResp.status}).`;
                    throw new Error(msg);
                }
            ...
            this.isAnalyzing = false;
            // Ensure loading spinner is hidden even if request hangs/fails
    ...
    deriveBiasesFromText(text, provided, knowledgeBaseBiases = []){
        const normalizeBias = (entry) => {
            ...
            const counter = Array.isArray(entry.countermeasures)
                ? entry.countermeasures.filter(Boolean).map(toText).filter(Boolean)
                : Array.isArray(entry.actions)
                    ? entry.actions.filter(Boolean).map(toText).filter(Boolean)
                    : [];
